...
gem install bundler
bundle install
git clone --bare https://$GITHUB_ACTOR:$INPUT_GITHUB_TOKEN@github.com/$GITHUB_REPOSITORY.git --branch=gh-pages $MY_JEKYLL_SITE
...
git commit -m "Deploy!"
git --git-dir=gh-pages --work-tree=$MY_JEKYLL_SITE push origin
